	...
	difference ()
	{
		intersection ()
		{
			union()
			{
				...
				cylinder ($fn=number_of_teeth*4, r1=root_cone_full_radius,r2=0,h=apex_to_apex,$fn=bacfn);
...
//				for (i = [1:1])
			...
			if (finish == bevel_gear_back_cone)
			{
				...
				cylinder (
					$fn=number_of_teeth*4, 
					r1=back_cone_end_radius,
					r2=back_cone_full_radius*2,
					h=apex_to_apex + back_cone_descent,$fn=bacfn);
			}
			else
			{
		...
		if (finish == bevel_gear_back_cone)
		{
			...
			cylinder (
				...
				h=face_cone_height + face_cone_descent+pitch_apex,$fn=bacfn);
		}

		translate ([0,0,pitch_apex - apex_to_apex])
		cylinder (r=bore_diameter/2,h=apex_to_apex,$fn=bacfn);
	}	
}
...
//		half_thick_angle);
	...
	res=(involute_facets!=0)?involute_facets:($fn==0)?5:$fn/4;
   	...
   	echo ("involute_bevel_gear_tooth::Res:", res);
		...
		union ()
		{
			difference ()
			{
				...
				if (gear_thickness < rim_thickness)
					...
					cylinder (r=rim_radius,h=rim_thickness-gear_thickness+1,$fn=bacfn);
			}
			if (gear_thickness > rim_thickness)
				cylinder (r=rim_radius,h=gear_thickness,$fn=bacfn);
			if (hub_thickness > gear_thickness)
				translate ([0,0,gear_thickness])
				cylinder (r=hub_diameter/2,h=hub_thickness-gear_thickness,$fn=bacfn);
		}
		translate ([0,0,-1])
		cylinder (
			r=bore_diameter/2,
			h=2+max(rim_thickness,hub_thickness,gear_thickness),$fn=bacfn);
		if (circles>0)
		{
			for(i=[0:circles-1])	
				rotate([0,0,i*360/circles])
				translate([circle_orbit_diameter/2,0,-1])
				cylinder(r=circle_diameter/2,h=max(gear_thickness,rim_thickness)+3,$fn=bacfn);
		}
	}
}
	...
	involute_facets)
{
	union()
	{
		rotate (half_thick_angle) circle ($fn=number_of_teeth*4, r=root_radius);
	...
	involute_facets)
{
	...
	res=(involute_facets!=0)?involute_facets:($fn==0)?30:$fn/4;

    	echo ("involute_gear_tooth::Res:", res);
...
module test_backlash ()
{
	...
	color([0,0,128,0.5])
	translate([0,0,-5])
	cylinder ($fn=20,r=backlash / 4,h=25,$fn=bacfn);
}
...
module meshing_double_helix ()
{
  ...
  difference()
    ...
    translate([0,0,+3])
        mirror([0,0,1])
      cylinder(r1=(10-0.4)/2,r1=(10-0.4)/2+2,h=8,center=true,$fn=120);
    }
  ...
  difference()
	...
	mirror ([0,1,0])
	    ...
	    rotate(360/(12*2),[0,0,1])
	      test_double_helix_gear (teeth=12,circles=8);
    //cone fillet
    translate([spacing,0,+3])
        mirror([0,0,1])
      cylinder(r1=(10-0.4)/2,r1=(10-0.4)/2+2,h=8,center=true,$fn=120);
     }
...
module bracket_thing()
  ...
  difference()
    ...
    union()
      {
      //large axel
      cylinder(r=(10-0.4)/2,h=12,center=true,$fn=120);
      //cone fillet
      translate([0,0,-2.45])
        cylinder(r1=(10-0.4)/2,r1=(10-0.4)/2+2,h=8,center=true,$fn=120);
      ...
      translate([0,0,-6.5])
        {
        cylinder(r=(32)/2,h=3,center=true,$fn=360);
	    translate ([spacing,0,0]) cylinder(r=(24)/2,h=3,center=true,$fn=360);
        }
      translate([0,0,-6.5-3/2])
        {
        translate([0,1,0])
          rotate(-9.4,[0,0,1])
            cube([spacing,15,3]);
    ...
    //Hole to reduce printing
    translate([spacing/2,0,-6.5]) cylinder(r=(13)/2,h=9,center=true,$fn=360);
    //tab to lift it off
    translate([-23.5,-30/2,-6])
      rotate(50,[0,1,0])
        cube([10,30,6]);
      ...
      difference()
        {
        translate([0,0,6.7-1.7]) cylinder(r=(12)/2,h=1);
        translate([0,0,6.6-1.7]) cylinder(r1=(10.2)/2,r2=(8)/2,h=1.2,$fn=360);
        }        
